...
- 阶段计划：
  ...
  - [ ] C. 回合外响应框架（ask 原语的上层应用）：
    - 求桃按座次：濒死结算重构，从濒死角色起按行动顺序依次询问桃
    - 无懈响应链：按行动顺序询问，后手无懈抵消先手（last-wins），简化 AI 保留"只保护自己"
  - [ ] D. 八卦阵：响应闪时可选判定代替（接入杀/万箭的闪响应 options；判定红视为出闪）
...
- 装备：全部 19 张（武器 9 + 防具 2 + 马 6）；牌堆由 `src/standardDeck.json` 数据驱动（一副 108 张）
  ...
  - 白板（效果待对应系统）：八卦阵（判定出闪）、青釭剑（无视防具）、丈八蛇矛（转化）、方天画戟（多目标）
...
- 注：八卦阵（判定出闪）依赖杀响应流程；青釭剑依赖防具模型；青龙偃月刀/贯石斧已实现
