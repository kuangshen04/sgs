...
- [ ] 八卦阵：判定红视为一张虚拟闪（0 牌转化的一种）——响应窗口内主动选择"用八卦阵判定"还是"出闪"，判定失败仍可再出闪；依赖本系统 + 响应窗口主动选择
- 解锁：关羽、赵云、甘宁、甄宓、大乔、华佗、貂蝉（离间）、丈八蛇矛、八卦阵
...
- [ ] 防具效果：八卦阵（判定出闪；属 0 牌转化 + 响应窗口主动选择，依赖 #3，暂缓）
- [ ] 锁定技标记：区分锁定技与主动/触发技能
...
- [ ] 统一移动模型：牌堆/弃牌堆并入 moveCards（`CardLocation` 位置句柄；顶/底是取放策略不是位置；摸牌/判定/取回/洗牌都走移动事件）
...
- [ ] 杀响应完整化：响应询问窗口（玩家选择）；八卦阵移入 #3（依赖虚拟牌语义）
...
- 阶段计划：
  ...
  - 暂缓：八卦阵（涉及响应窗口主动选择 + 虚拟牌 + 判定失败可再出闪，依赖 #3 转化牌/虚拟牌语义，不做进本 todo）
...
- 基本：杀、闪、桃
- 锦囊：无中生有、决斗、南蛮入侵、万箭齐发、桃园结义、五谷丰登（简化）、顺手牵羊、过河拆桥、借刀杀人、无懈可击（简化 AI）
- 延时：乐不思蜀、闪电
- 装备：全部 19 张（武器 9 + 防具 2 + 马 6）；牌堆由 `src/standardDeck.json` 数据驱动（一副 108 张）
  - 有实际效果：诸葛连弩、仁王盾、麒麟弓、寒冰剑、雌雄双股剑、青龙偃月刀（抵消后再出杀）、贯石斧（抵消后弃牌命中）
  - 白板（效果待对应系统）：八卦阵（判定出闪，依赖 #3 转化牌）、青釭剑（无视防具）、丈八蛇矛（转化）、方天画戟（多目标）
  - 马：绝影、的卢、爪黄飞电（防御马）；赤兔、大宛、紫骍（进攻马，槽位距离修正已生效）

### 待实现

- [x] 乐不思蜀（延时锦囊）
- [x] 闪电（延时锦囊）
- [x] 借刀杀人（装备区 + 攻击距离，已实现简化 AI）
- 注：八卦阵（判定出闪）依赖 #3 转化牌（虚拟闪）+ 响应窗口主动选择；青釭剑依赖防具模型；青龙偃月刀/贯石斧已实现
...
- 曹操（奸雄）、刘备（仁德）、夏侯惇（刚烈）、司马懿（反馈/鬼才）、郭嘉（遗计/天妒）、甄宓（洛神）、张辽（突袭）、黄月英（集智/奇才）、华佗（青囊）、孙权（制衡）、周瑜（英姿/反间）、貂蝉（闭月）、张飞（咆哮）、许褚（裸衣）、马超（马术/铁骑）、诸葛亮（空城）、陆逊（谦逊/连营）、黄盖（苦肉）、孙尚香（结姻/枭姬）、吕布（无双）、大乔（流离）

### 待实现（括号内为关键依赖）

- 甄宓：倾国（转化）
- 诸葛亮：观星（准备阶段/牌堆操作）
- 大乔：流离（已实现，targeting 事件内转移目标）；国色（转化）
- 关羽：武圣（转化）
- 赵云：龙胆（转化）
- 甘宁：奇袭（转化）
- 华佗：急救（转化）
