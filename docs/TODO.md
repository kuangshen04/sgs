...
- **模块分层**（ADR-0010）：`EffectCommon.skill`/`equipType` 这两个倒查字段是否改成
  "注册时带归属谓词"（代价 = 归属规则从引擎挪到装配点；按 ADR-0010 技能定义与实现不可分，
  这个需求可能根本不存在）；依赖边界的**强制闸**（eslint import 边界 / dependency-cruiser）
  何时加——现在靠人工核对，等出现第二次越界再说。
