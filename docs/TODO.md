...
- 有实际效果：诸葛连弩、仁王盾、麒麟弓、寒冰剑、雌雄双股剑、青龙偃月刀、贯石斧、6 匹马（槽位距离修正）
- 白板（效果待对应系统）：
  - 青釭剑（无视防具，依赖防具模型）
  - 八卦阵（判定出闪：0 牌转化 + 响应窗口主动选择，依赖 #3，暂缓）
  - 丈八蛇矛（两张手牌当杀，转化 #3）
...
- 转化类（依赖 #3）：甄宓·倾国、大乔·国色、华佗·急救、貂蝉·离间（0 牌转化）
- 诸葛亮·观星（准备阶段：牌堆顶操作 + 排序选择，依赖 #9）
- 主公技（依赖回合外响应框架/势力）：曹操·护驾、刘备·激将、孙权·救援

### 已实现但与标包有差距（简化版说明）

- 反间：直接给牌并造成伤害（原：目标猜花色，花色不同才伤害）
- 制衡：弃置全部手牌（原：任选任意张）
- 仁德：固定给出 2 张（原：任意张，本回合给出第二张起回血）
- 遗计：摸牌全给自己（原：观看牌堆顶 2 张并分配任意角色）
- 青囊：只给自己（原：任意受伤角色；AI 简化）
