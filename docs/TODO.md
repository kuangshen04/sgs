...
2. **frequency 核心词 / 锁定技标记**——锁定技 = Compulsory 标签，语义 = 自动发动（供前端消费）；
   仅当规则文本声明"锁定技"时由内容层添加，引擎不根据效果形态推导（TODO #4）。
   - 真实用例：空城/谦逊/奇才/马术/无双/咆哮等现有锁定技。依赖 1。
   - 验收：#4 勾选；同一时点排序（第 7 项）依赖本项。
3. **技能失效 / 复原**——临时失去效果但**不触发生命周期钩子**；含抗性（锁定技抗性等）在失效判断之前。
   - 第一个真实用例：青釭剑（令目标防具技能失效）。依赖 1。
   - 验收：青釭剑从白板变可用（TODO 二、装备"青釭剑"勾选）。
4. **装备技能建模**——`equipTrigger` 并入统一效果模型，保留"生命周期 + 局内存储"（第九节 9.5）。
   - 决策点（伴生）：装备区是否收进位置模型（见下方伴生事项）。真实用例：现有 17 张装备 + 青釭剑。
...
7. **排序显式化**（演进 5.2 红线）——座次主排序 + 同优先级玩家手选（ask 原语已有）+
   锁定技自动发动不询问（依赖第 2 项）。行为变化需显式说明 + 机械更新测试。
8. **技能 info 规则文本**——`SkillDef.info` / `CardDef.info` 纯数据字段，内容源 = docs 导出 JSON（TODO #5 一.5）。
   低风险，可随时插入。
9. （可选）**技能实例化（定义静态 + 局内实例）**——演进 9.2 已确认方向；触发条件 = 出现需要局内状态的效果
   （限定技已用 / 失效标记 / 次数）。第 3、5 项若需要即做，否则留阶段 5（第九节开放问题 2）。
...
> 技能与效果建模（阶段 3 拆解见上：effect 收口 / 锁定技 / 失效 / 装备 / 国色 / 离间 / 排序 / info）→
