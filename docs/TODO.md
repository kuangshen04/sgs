...
- [ ] 发动词汇三轴（勿混为一谈，详见演进 9.2）：
  - effect 级 `frequency.auto`（自动发动）：绑 effect，作用 = 前端多一个"自动发动"按钮（**现在不做**，仅留位置）；
  - effect 级 `forced`（强制发动）：绑触发技的 effect，作用 = 不进行"是否发动"的询问（本项落地）；
...
2. **发动词汇三轴落地**（原"锁定技标记"，TODO #4）——三件事分属三类对象，勿混为一谈：
   ...
   - 真实用例：空城/谦逊/奇才/马术/无双/咆哮等现有锁定技。依赖 1。
   - 验收：#4 勾选、`forced` + `Compulsory` 可用（`auto` 仅占位）；第 6 项排序依赖本项。
3. **技能失效 / 复原**——临时失去效果但**不触发生命周期钩子**；失效判断**之前**先查 `Compulsory` 抗性
   （与第 2 项配套）。依赖 1、2。
   - 第一个真实用例：青釭剑（令目标防具技能失效）。验收：青釭剑从白板变可用（TODO 二、装备"青釭剑"勾选）。
4. **判定区转化身份 / 国色**——长生命周期 UsedCard 与 CardArea 的对应关系（阶段 2 ✓ 已就绪）。
   - 真实用例：国色（标包唯一）。依赖 1 + 阶段 2。
5. **0 牌转化 + 不可被无懈（离间）**——依赖转化系统与无懈窗口；与国色同为标包收尾内容。
...
7. **技能 info 规则文本**——`SkillDef.info` / `CardDef.info` 纯数据字段，内容源 = docs 导出 JSON（TODO 一.5）。
   低风险，可随时插入。
8. **（延后）装备技能建模**——等**武将技能迁移完成后**再看（内容可能后续会改）；届时一并决定
   `equipTrigger` 归位形态与"装备区是否收进位置模型"（见下方伴生事项）。用例：现有 17 张装备 + 青釭剑（第九节 9.5）。
9. （可选）**技能实例化（定义静态 + 局内实例）**——演进 9.2 已确认方向；触发条件 = 出现需要局内状态的效果
   （限定技已用 / 失效标记 / 次数）。第 3 项若需要即做，否则留阶段 5（第九节开放问题 2）。
