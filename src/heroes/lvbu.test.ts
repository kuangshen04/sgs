...
describe('无双（吕布锁定技）', () => {
  ...
  it('吕布使用杀 → 目标需两张闪：只有一张则命中', async () => {
    ...
    expect(target.hand.cards.length).toBe(1);   // 闪已打出，剩桃
  });
...
describe('无双②（决斗）', () => {
  it('吕布使用决斗 → 对方每次响应需两张杀', async () => {
    ...
    expect(target.hand.cards.length).toBe(1); // 剩桃，杀已打出
  });
  ...
  it('吕布是决斗目标 → 自己每次响应只需一张杀，对方需两张', async () => {
    ...
    expect(lvbu.hand.cards.length).toBe(1); // 剩桃
  });
  ...
  it('双方都有两张杀 → 先耗尽的一方失败', async () => {
    ...
    expect(target.hp).toBe(hpBefore - 1);
    expect(target.hand.cards.length).toBe(1); // 剩桃
    expect(lvbu.hand.cards.length).toBe(3);   // 剩三张杀
  });
  ...
  it('双方都是吕布 → 双方每次响应都需两张杀', async () => {
    ...
    expect(lvbuB.hand.cards.length).toBe(1);
    expect(lvbuA.hand.cards.length).toBe(1);
  });
});
