 ...
 * 因果判定走 DamageEventData.card（规则层在"牌直接造成伤害"处显式赋值，adr/0001）——
 * 不再经 getParent('useCard') 推断：刚烈等在 damage.after 内发起的反击伤害
 * 嵌套于原伤害之下，栈查询会把反击伤害误归给原杀/决斗（已知问题，本修复针对它）。
 *
 * **读 UC 拿实体牌**（adr/0003）：造成伤害的牌此刻是一条停在处理区的 UC，
...
const jianxiongContent = async (game: Game, event: GameEvent<any>, owner: Player): Promise<void> => {
  ...
  console.log(
    `  ✨${owner.name} 发动【奸雄】！获得造成伤害的 ${cardEmoji(game, uc.type)} ` +
    `(${cardFaceText(uc)})`,
  );
};

// ── 装配（显式注册进容器；参数 c = 装配期容器）──────────────────────
export function installCaocao(c: Container): void {
  c.skills.define({
    name: '奸雄',
    effects: [{
      form: 'triggered',
