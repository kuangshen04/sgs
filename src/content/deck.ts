...
// 生成 Card[]。卡牌定义由内容包装配进容器，此处只做数据映射。
// ============================================================
 ...
 * `ruleSet` = 已装配好的规则集（卡牌名 → 定义；未注册的牌跳过并警告，防止漏注册）。
 */
export function buildStandardDeck(ruleSet: RuleSet, startId = 1): Card[] {
  const deck: Card[] = [];
  let next = startId;
  for (const entry of deckData) {
    ...
    const def = ruleSet.cards.get(type);
    if (!def) {
