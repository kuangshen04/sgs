// ============================================================
// 触发排序 — 座次主排序 + 同角色多候选由该角色选择顺序（adr/0005 红线）
//
...
import { standardGame } from '../test-utils.js';
...
const TEST_HEROES = ['测试·时序A', '测试·时序B', '测试·多候选', '测试·条件重算'].map((name) => ({
  name, maxHp: 4, sex: 'male' as const, group: '群' as const, skills: [name],
}));
...
async function fire(g: Game, timing: string): Promise<void> {
...
describe('触发排序：座次主排序（从当前回合角色起，按行动顺序）', () => {
  it('当前回合角色是座次 1 → 其效果先于座次 0 的角色', async () => {
    const g = testLineup(['测试·时序A', '测试·时序B', '刘备']);
